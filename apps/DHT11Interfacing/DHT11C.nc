	...
	atomic{			
		if(bit_count!=40){
			...
			for(i=0;i<40;i++){
				if(measurement[i] == 1){
					if(i<8)
						SET_BIT(humid_int,(7-i));
					else if(i > 8 && i < 16)
						SET_BIT(humid_dec,(15-i));
					else if(i > 16 && i < 24 )
						SET_BIT(temperature_int,(23-i));
					else if(i > 24 && i < 32)
					SET_BIT(temperature_dec,(31-i));	
					else if(i >32 && i< 40)
						SET_BIT(checksum,(39-i));		
				}
			}
			call Leds.led0On();				
			humid_int = measurement;
			//memcpy(&humid_int,&measurement,8);
			//memcpy(&humid_dec,&measurement[8],8);
			//memcpy(&temperature_int,&measurement[16],8);
			//memcpy(&temperature_dec,&measurement[24],8);
			//memcpy(&checksum,&measurement[32],8);

		/*	
			humid_dec         = (measurement>>24)&0x0000f;
			temperature_int   = (measurement>>16)&0x0000f;
			temperature_dec   = (measurement>>8)&0x0000f;
			checksum          = measurement&0x0000f;
		*/
		}
