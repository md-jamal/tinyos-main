...
//  uses interface StdControl as RoutingControl;
 // uses interface StdControl as DAOControl;
  ...
  void sendMsg(struct in6_addr ip,uint8_t type)
  {
	...
	if(type==NS)
	{
		...
		/*
			...
			inet_ntop6(&pkt.ip6_hdr.ip6_dst,buff,64);
						call Lcd.lcdStr(buff);
		#endif
*/
...
/*******************************************EVENTS*************************************************************************/
   ...
   event void IP_RA.recv(struct ip6_hdr *hdr, void *packet, 
	...
	adv=(router_advt *)packet;
			...
			//call Lcd.lcdClear();
			//call Lcd.lcdGotoxy(1,2);
			//call Lcd.lcdStr("RA recv");

			if(!prefix_option){
				call Lcd.lcdClear();
				call Lcd.lcdGotoxy(1,2);
				call Lcd.lcdStr("No PIO");
			}
			if(!opt){
				call Lcd.lcdClear();
				call Lcd.lcdGotoxy(1,2);
				call Lcd.lcdStr("No SLLAO");
				}

			if(!abro){
				call Lcd.lcdClear();
				call Lcd.lcdGotoxy(1,2);
				call Lcd.lcdStr("No ABRO");
				}
	#endif
...
//This created a big issue solved by writing the next line memcpy(&TENTATIVE_ADDRESS,&prefix_option->prefix_information.prefix,prefix_option->prefix_information.prefix_length);		
	memcpy(&TENTATIVE_ADDRESS,&prefix_option->prefix_information.prefix,sizeof(struct in6_addr));

			 //inet_pton6("2001::", &TENTATIVE_ADDRESS);
				...
				//call IPAddress.setAddress(&TENTATIVE_ADDRESS);			
                  ...
                  size_t len, struct ip6_metadata *meta)
   {
	...
	if(reg!=0)		//this may be reply of ARO registration
	{
		...
		{	
			...
			call IPAddress.setAddress(&na->target);
